...
const defaultTemplates = [
  { id: "t1", name: "Deep work block", target: 1, unit: "", xp: 40 },
  { id: "t2", name: "Read", target: 20, unit: "pages", xp: 30 },
  { id: "t3", name: "Move your body", target: 1, unit: "", xp: 25 },
];
...
export default function QuestTracker() {
    ...
    (async () => {
      try {
        ...
        if (saved) {
          ...
          setTaskStreaks(parsed.taskStreaks || {});
        }
      } catch (e) {
  ...
  const saveAll = useCallback(
    (updates) => {
      ...
      setLastCompletedDate(next.lastCompletedDate);
      setTaskStreaks(next.taskStreaks);
      persist(next);
    },
    [templates, dailyLogs, xp, streak, lastCompletedDate, taskStreaks, persist]
  );
  ...
  const allComplete = templates.length > 0 && mainProgress >= 1;
  ...
  const updateTaskProgress = (taskId, delta) => {
    ...
    let nextLastCompleted = lastCompletedDate;
    const yesterday = dateOffset(today, -1);

    // per-task daily streak
    const ts = taskStreaks[taskId] || { streak: 0, lastDate: null };
    let nextTaskStreaks = taskStreaks;
    if (!wasComplete && nowComplete && ts.lastDate !== today) {
      const newTs = {
        streak: ts.lastDate === yesterday ? ts.streak + 1 : 1,
        lastDate: today,
      };
      nextTaskStreaks = { ...taskStreaks, [taskId]: newTs };
    } else if (wasComplete && !nowComplete && ts.lastDate === today) {
      const revertedStreak = Math.max(0, ts.streak - 1);
      const newTs = {
        streak: revertedStreak,
        lastDate: revertedStreak > 0 ? yesterday : null,
      };
      nextTaskStreaks = { ...taskStreaks, [taskId]: newTs };
    }

    // recompute all-complete for today with the new log
    const sumTarget = templates.reduce((s, t) => s + t.target, 0);
    const sumCurrent = templates.reduce(
      (s, t) => s + Math.min(t.id === taskId ? nextVal : todayLog[t.id] || 0, t.target),
      0
    );
    const nowAllComplete = sumTarget > 0 && sumCurrent >= sumTarget;

    if (nowAllComplete && lastCompletedDate !== today) {
      if (lastCompletedDate === yesterday) {
        nextStreak = streak + 1;
      } else {
        nextStreak = 1;
      }
      nextLastCompleted = today;
      nextXp += 25;
      showToast(`Day complete — streak now ${nextStreak} \u{1F525}`);
    } else if (!nowAllComplete && lastCompletedDate === today) {
      const revertedStreak = Math.max(0, streak - 1);
      nextStreak = revertedStreak;
      nextLastCompleted = revertedStreak > 0 ? yesterday : null;
      nextXp = Math.max(0, nextXp - 25);
    }

    if (!wasComplete && nowComplete && !nowAllComplete) {
      const streakNote =
        nextTaskStreaks[taskId]?.streak > 1 ? ` — ${nextTaskStreaks[taskId].streak} day streak` : "";
  ...
  const getMainDayStatus = useCallback(
    (dateStr) => {
      ...
      if (!log) return "empty";
      const sumTarget = templates.reduce((s, t) => s + t.target, 0);
      const sumCurrent = templates.reduce((s, t) => s + Math.min(log[t.id] || 0, t.target), 0);
  ...
  const addTask = () => {
    ...
    const nextTemplates = [
      ...templates,
      { id, name, target, unit: newTaskUnit.trim(), xp: xpVal },
    ];
    saveAll({ templates: nextTemplates });
    setNewTaskName("");
    setNewTaskTarget("1");
    ...
    <div style={styles.app}>
      ...
      <div style={styles.container}>
        ...
        <div style={styles.taskListHeader}>
          <span style={styles.sectionLabel}>DAILY QUESTS</span>
        ...
        {showAdd && (
          <div style={styles.addForm}>
            ...
            />
            <button style={styles.confirmBtn} onClick={addTask}>
        ...
        <div style={styles.taskList}>
          {templates.length === 0 && (
            <div style={styles.emptyState}>No quests yet. Add one to begin your log.</div>
          )}
          {templates.map((task) => {
            const current = todayLog[task.id] || 0;
            const pct = Math.min(100, (current / task.target) * 100);
            const complete = current >= task.target;
            const tStreak = taskStreaks[task.id]?.streak || 0;
            return (
